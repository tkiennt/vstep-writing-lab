      ...
      <div className="flex flex-col h-full bg-background">
        ...
        <header className="h-16 bg-card border-b border-border flex items-center justify-between px-8 shrink-0">
           <h1 className="text-xl font-bold text-foreground flex items-center gap-2">
              <BrainCircuit className="w-5 h-5 text-indigo-400" /> System Prompt Builder
           </h1>
           <div className="flex items-center gap-3">
              <Button 
                variant={testMode ? "default" : "outline"} 
                onClick={() => setTestMode(!testMode)}
                className={`flex items-center gap-2 rounded-xl transition-all ${testMode ? 'bg-indigo-600 hover:bg-indigo-700 text-white border-transparent' : 'border-border text-foreground'}`}
              >
        ...
        <main className="flex-1 overflow-y-auto w-full p-8">
           <div className={`mx-auto grid gap-8 pb-20 transition-all duration-500 ease-in-out ${testMode ? 'grid-cols-2 max-w-full' : 'grid-cols-1 max-w-4xl'}`}>
              ...
              {/* Prompt Editor */}
              <div className="bg-card rounded-3xl border border-border shadow-sm overflow-hidden flex flex-col h-[700px] relative transition-all">
                 <div className="px-6 py-4 bg-slate-900 flex items-center justify-between shrink-0">
                    ...
                    <span className="text-xs font-mono text-slate-400">system_vstep_ai_v2.txt</span>
                 </div>
              ...
              {testMode && (
                 <div className="bg-muted/20 rounded-3xl border border-border shadow-inner overflow-hidden flex flex-col h-[700px] animate-in slide-in-from-right-8 duration-300">
                    
                    {/* Top: Input Mock Essay */}
                    <div className="p-6 border-b border-border flex-1 flex flex-col bg-card">
                       <h3 className="font-bold text-foreground text-sm mb-3">1. Paste Mock Essay ({`{{ STUDENT_ESSAY }}`})</h3>
                       <textarea 
                          placeholder="Paste a student essay here to test the prompt against..." 
                          className="flex-1 w-full bg-background border border-border rounded-xl p-4 text-sm font-serif outline-none focus:ring-2 focus:ring-indigo-500/20 resize-none mb-4 text-foreground placeholder:text-muted-foreground"
                    ...
                    <div className="p-6 bg-slate-900 flex-1 flex flex-col relative">
                       ...
                       <div className="flex-1 bg-black/50 border border-slate-800 rounded-xl p-4 overflow-auto relative">
                          {testLoading ? (
                             ...
                             <p className="text-slate-600 font-mono text-xs text-center mt-20">Click &apos;Test Prompt&apos; to see results</p>
                          )}
                       </div>
                    </div>
