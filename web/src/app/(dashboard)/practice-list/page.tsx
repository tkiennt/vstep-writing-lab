      ...
      <div className="space-y-4 mb-10">
        <div className="flex items-center gap-4 mb-3">
          <h2 className={`text-sm font-black px-3 py-1 rounded-lg border ${
            level === 'C1' ? 'border-red-500/30 text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-500/10' : 
            level === 'B2' ? 'border-amber-500/30 text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-500/10' : 
            'border-emerald-500/30 text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-500/10'
          }`}>VSTEP {level}</h2>
          <div className="h-px flex-1 bg-slate-200 dark:bg-slate-700/60"></div>
        </div>

        <div className="grid gap-4">
          {items.map((p) => (
            <div key={p.id} className="group flex flex-col md:flex-row gap-6 p-6 rounded-2xl border border-slate-200 dark:border-slate-700/50 bg-white dark:bg-slate-800 hover:border-emerald-500/30 hover:-translate-y-0.5 transition-all duration-200 relative overflow-hidden shadow-sm">
              <div className="flex-1">
                <div className="flex flex-wrap items-center gap-2 mb-3">
                  <span className={`px-2.5 py-1 rounded-md text-[10px] font-black uppercase tracking-wider ${
                    p.taskType.toLowerCase() === 'task1' ? 'bg-indigo-100 dark:bg-indigo-500/15 text-indigo-600 dark:text-indigo-400' : 'bg-fuchsia-100 dark:bg-fuchsia-500/15 text-fuchsia-600 dark:text-fuchsia-400'
                  }`}>
              ...
              <div className="flex items-center justify-between md:justify-end md:w-48 border-t md:border-t-0 md:border-l border-slate-200 dark:border-slate-700/50 pt-5 md:pt-0 md:pl-6">
                <Link href={`/practice/${p.id}`} className="w-full">
                  <Button className="w-full bg-emerald-600 hover:bg-emerald-700 dark:bg-vstep-dark dark:hover:bg-emerald-950 text-white rounded-xl h-14 font-black text-base flex items-center justify-center gap-3 transition-all active:scale-95">
  ...
  return (
    <div className="min-h-full bg-slate-50 dark:bg-slate-950">
      <div className="space-y-5 max-w-6xl mx-auto pb-16 px-6 sm:px-8 pt-5">
        ...
        {/* Hero */}
        <div className="relative bg-emerald-50 dark:bg-slate-800 p-8 md:p-10 rounded-2xl border border-emerald-200 dark:border-slate-700/50 overflow-hidden group">
          <div className="relative z-10 max-w-2xl">
            <div className="inline-flex items-center gap-2 px-3.5 py-1.5 bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 rounded-full text-xs font-black uppercase tracking-widest mb-5 border border-emerald-500/20">
              <Sparkles className="w-3.5 h-3.5" /> {t('practiceList.hero.badge')}
            </div>
            <h1 className="text-3xl md:text-4xl font-black text-slate-900 dark:text-white tracking-tight leading-tight mb-3">
              {t('practiceList.hero.title')} <span className="text-emerald-600 dark:text-emerald-400">{t('practiceList.hero.titleHighlight')}</span>
            </h1>
          ...
          <div className="absolute top-1/2 right-10 -translate-y-1/2 hidden lg:block opacity-5 group-hover:opacity-10 transition-opacity">
            <Trophy className="w-40 h-40 text-emerald-500 dark:text-emerald-400" />
          </div>
        ...
        <div className="flex flex-col lg:flex-row justify-between lg:items-center gap-4 bg-white dark:bg-slate-800 p-3 rounded-2xl border border-slate-200 dark:border-slate-700/50 sticky top-4 z-30 shadow-sm">
          ...
          <div className="relative shrink-0 flex-1 md:max-w-xs xl:max-w-sm">
            ...
            <input 
              ...
              className="w-full pl-11 pr-5 py-2.5 bg-slate-100 dark:bg-slate-700 border border-slate-200 dark:border-slate-600 rounded-xl text-sm font-medium text-slate-900 dark:text-slate-200 placeholder:text-slate-400 dark:placeholder:text-slate-500 outline-none focus:border-emerald-500/50 focus:ring-2 focus:ring-emerald-500/10 transition-all" 
            />
          </div>
        </div>

        {loading ? (
          <div className="flex flex-col items-center justify-center py-24 bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700/50">
            ...
            <p className="text-slate-400 dark:text-slate-500 font-black mt-6 text-sm uppercase tracking-widest animate-pulse">{t('practiceList.loading')}</p>
          </div>
        ) : filteredPrompts.length === 0 ? (
          <div className="text-center py-24 bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700/50">
            <div className="w-20 h-20 bg-slate-100 dark:bg-slate-700 rounded-full flex items-center justify-center mx-auto mb-5 border border-slate-200 dark:border-slate-600">
              <Search className="w-8 h-8 text-slate-400 dark:text-slate-500" />
            </div>
            <h3 className="text-xl font-black text-slate-900 dark:text-slate-100 mb-2">{t('practiceList.empty.title')}</h3>
            <p className="text-slate-500 dark:text-slate-400 font-medium max-w-sm mx-auto mb-6">{t('practiceList.empty.subtitle')}</p>
