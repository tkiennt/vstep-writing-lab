...
import { gradeEssay, GradeEssayRequest } from '@/lib/api';
...
import { ChevronUp, ChevronDown, CheckCircle2, BookOpen, Layers, Loader2 } from 'lucide-react';
      ...
      {isSubmitting && (
        <div className="absolute inset-0 z-50 bg-white/70 backdrop-blur-md flex items-center justify-center rounded-3xl animate-in fade-in">
          <div className="bg-white p-10 rounded-3xl shadow-2xl flex flex-col items-center max-w-md text-center border border-emerald-100">
            ...
            <p className="text-slate-500 font-medium mt-3 mb-8">Xin đợi trong giây lát, kết quả phân tích chi tiết sẽ có sau 10-15 giây nữa. Vui lòng không rời khỏi trang.</p>
            
            {/* Fake progress bar animation */}
            <div className="w-full h-2 bg-slate-100 rounded-full overflow-hidden">
              <div className="h-full bg-emerald-500 rounded-full w-full animate-[progress_12s_ease-out_forwards]" />
            </div>
          </div>
        </div>
      )}
