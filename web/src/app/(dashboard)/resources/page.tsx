...
export default function ResourcesPage() {
  ...
  const handleCopy = (text: string) => {
    ...
    addToast('success', t('resources.copySuccess'));
  };
  ...
  const handleSpeech = (text: string) => {
    if ('speechSynthesis' in window) {
      ...
      addToast('error', t('resources.ttsError'));
    }
  };
    ...
    <div className="space-y-8 max-w-7xl mx-auto pb-12">
      ...
      <div>
        <h1 className="text-3xl font-extrabold text-slate-900 dark:text-white tracking-tight flex items-center gap-3">
           <Library className="w-8 h-8 text-emerald-600 dark:text-emerald-400" /> {t('resources.title')}
        </h1>
        <p className="text-slate-500 dark:text-slate-400 mt-2">{t('resources.subtitle')}</p>
      </div>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-3xl border border-slate-200 dark:border-slate-700/50 shadow-sm overflow-hidden flex flex-col md:flex-row min-h-[600px]">
         ...
         <div className="w-full md:w-64 bg-slate-50 dark:bg-slate-900/50 border-b md:border-b-0 md:border-r border-slate-100 dark:border-slate-700/50 p-4 shrink-0">
            <div className="space-y-2">
               ...
               >
                  <BookMarked className="w-4 h-4" /> {t('resources.tabs.vocab')}
               </button>
               <button 
               ...
               >
                  <MessageSquareQuote className="w-4 h-4" /> {t('resources.tabs.structures')}
               </button>
               <button 
               ...
               >
                  <FileText className="w-4 h-4" /> {t('resources.tabs.samples')}
               </button>
            </div>
         </div>
         ...
         <div className="flex-1 p-6 lg:p-10 overflow-y-auto">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-between items-center mb-8">
               <h2 className="text-xl font-bold text-slate-900 dark:text-white uppercase tracking-widest text-xs">
                  {activeTab === 'vocab' && t('resources.headers.vocab')}
                  {activeTab === 'structures' && t('resources.headers.structures')}
                  {activeTab === 'samples' && t('resources.headers.samples')}
               </h2>
               <div className="flex items-center gap-3 w-full sm:w-auto">
                  <div className="relative flex-1 sm:w-64">
                    <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 dark:text-slate-500" />
                    <input type="text" placeholder={t('resources.search')} className="w-full pl-9 pr-4 py-2 bg-slate-50 dark:bg-slate-700 border border-slate-200 dark:border-slate-600 rounded-lg text-sm outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 font-medium text-slate-900 dark:text-slate-200 placeholder:text-slate-400 dark:placeholder:text-slate-500" />
                  </div>
               ...
               <div className="space-y-6 max-w-3xl">
                  <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl p-6 shadow-sm hover:border-emerald-300 dark:hover:border-emerald-500/30 transition-all group">
                     <div className="flex justify-between items-start mb-4">
                        <div>
                           <span className="text-[10px] font-black uppercase tracking-widest text-emerald-600 dark:text-emerald-400 mb-1 block">{t('resources.structures.contrastIntro.label')}</span>
                        ...
                        <Button variant="ghost" size="sm" onClick={() => handleCopy("While [Fact], I believe that [Opinion].")} className="text-slate-400 dark:text-slate-500 hover:text-emerald-600 dark:hover:text-emerald-400 opacity-0 group-hover:opacity-100 transition-opacity">
                           <Copy className="w-4 h-4 mr-1"/> {t('resources.copy')}
                        </Button>
                     </div>
                     <p className="text-sm text-slate-600 dark:text-slate-400 mb-4">{t('resources.structures.contrastIntro.description')}</p>
                  ...
                  <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl p-6 shadow-sm hover:border-blue-300 dark:hover:border-blue-500/30 transition-all group">
                     <div className="flex justify-between items-start mb-4">
                        <div>
                           <span className="text-[10px] font-black uppercase tracking-widest text-blue-600 dark:text-blue-400 mb-1 block">{t('resources.structures.advantagePresentation.label')}</span>
            ...
            {activeTab === 'samples' && (
               <div className="space-y-6">
                 <div className="flex items-center gap-3 mb-2">
                   ...
                   <div>
                     <h3 className="text-lg font-black text-slate-900 dark:text-white tracking-tight">{t('resources.samples.annotatedTitle')}</h3>
                     <p className="text-xs text-slate-500 dark:text-slate-400 font-medium">{t('resources.samples.annotatedSubtitle')}</p>
                   </div>
                 </div>
                 ...
                 <div className="flex flex-col lg:flex-row gap-6">
                   <div className="lg:w-[60%] p-6 bg-slate-50 dark:bg-slate-700/50 border border-slate-200 dark:border-slate-600 rounded-2xl font-serif text-base leading-loose text-slate-800 dark:text-slate-200">
                     <p>The phenomenon of global warming has become an{' '}
                       <span className="bg-yellow-100 dark:bg-yellow-500/20 border border-yellow-300 dark:border-yellow-500/30 rounded px-1 font-medium cursor-help" title={t('resources.samples.lexicalHighlightTooltip')}>
                   ...
                   <div className="lg:w-[40%]">
                     <div className="bg-white dark:bg-slate-800 border-2 border-amber-200 dark:border-amber-500/30 rounded-2xl p-5 shadow-sm relative">
                       ...
                       <p className="text-sm text-slate-600 dark:text-slate-400 leading-relaxed bg-slate-50 dark:bg-slate-700/50 p-4 rounded-xl border border-slate-100 dark:border-slate-600">
                         {t('resources.samples.lexicalHighlightDescription')}
                       </p>
                     </div>
                   </div>
                 </div>
                 ...
                 <div className="relative min-h-[350px]">
                   ...
                   <div className="absolute inset-0 flex items-center justify-center p-4">
                     <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-700 p-8 max-w-md w-full text-center">
                       ...
                       <h3 className="text-xl font-black text-slate-900 dark:text-white tracking-tight mb-3">{t('resources.pro.title')}</h3>
                       <p className="text-sm text-slate-500 dark:text-slate-400 leading-relaxed mb-6">
                         {t('resources.pro.subtitle')}
                       </p>
                       ...
                       <p className="text-[11px] text-slate-400 dark:text-slate-500 mt-3 font-medium">{t('resources.pro.price')}</p>
                     </div>
                   </div>
                 </div>
               </div>
