...
export const ScoreSummaryCard: React.FC<ScoreSummaryCardProps> = ({ score, cefrLevel }) => {
  const items = [
    ...
    { label: 'Tổ chức & Hành văn', val: score.organization, icon: BookOpen, color: 'text-amber-600', bg: 'bg-amber-50' },
    { label: 'Từ vựng', val: score.vocabulary, icon: BrainCircuit, color: 'text-purple-600', bg: 'bg-purple-50' },
    { label: 'Ngữ pháp', val: score.grammar, icon: Award, color: 'text-emerald-600', bg: 'bg-emerald-50' },
  ];

  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
      {items.map((item) => (
        <div key={item.label} className="bg-white rounded-3xl p-6 border border-slate-100 shadow-sm flex flex-col items-center text-center group hover:-translate-y-1 transition-all">
      ...
      <div className="lg:col-span-4 bg-gradient-to-r from-[#064e3b] to-emerald-800 rounded-[2.5rem] p-8 mt-4 flex flex-col md:flex-row items-center justify-between shadow-xl shadow-emerald-950/10">
